...
hold on
fig2 = plot3(xdata2, ydata2, zdata2, 'ro', 'MarkerSize', 10);
xlabel('x')
ylabel('y')
zlabel('z')
axis([-2, 2, -2, 2, 0, 4])
...
fig2.XDataSource = 'xdata2';
fig2.YDataSource = 'ydata2';
fig2.ZDataSource = 'zdata2';
...
while running
    ...
    %update foot booleans
    xdata2 = [rf(1)+rt(1), lf(1)+lt(1)];
    ydata2 = [rf(2)+rt(2)-0.25, lf(2)+lt(2)+0.25];
    zdata2 = [rf(3)+rt(3)+2, lf(3)+lt(3)+2];

    %turn off foot if off the ground
    if client.get('rp', 0) + client.get('rp', 1) + client.get('rp', 2) + client.get('rp', 3) < 2
        zdata2(1) = -1;
    end
    if client.get('lp', 0) + client.get('lp', 1) + client.get('lp', 2) + client.get('lp', 3) < 2
        zdata2(2) = -1;
    end
